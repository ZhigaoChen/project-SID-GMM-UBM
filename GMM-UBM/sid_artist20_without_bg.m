clear;
clc;
load 'result_artist20_16k_without_bg/gmm_ubm.mat';
for theta =0.04% 0 : 0.01 : 0.1
    disp('theta:');
    disp(theta);
    %best theta=0.04
    miss = 0;
    ...
    for ispeaker = [10001 : 10020]
        cepsmat = [];
        files = dir(['dataset/50_fold_artist20_whithout_bg/', num2str(ispeaker), '/test/*.wav']);
        for ifile = 1 : length(files)
            [d, sr] = wavread(['dataset/50_fold_artist20_whithout_bg/', num2str(ispeaker), '/test/', files(ifile).name]);
        ...
        for imodel = 10001 : 10020
            %disp('imodel====================');
            %disp(imodel);
            load(['result_artist20_16k_without_bg/gmm_speaker_', num2str(imodel), '.mat']);
    ...
    %disp(misstate ^ 2 + falseposrate ^ 2);
