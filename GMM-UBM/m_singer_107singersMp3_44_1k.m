...
for ispeaker = 10001 : 10055
    ...
    for ifile = 1 : length(files)
        [d, sr] = audioread(['dataset/107singersMp3_20_fold/', num2str(ispeaker), '/train/', files(ifile).name]);
        
        disp(['dataset/107singersMp3_20_fold/', num2str(ispeaker), '/train/', files(ifile).name]);
        disp('wavfile======up===ing==');
